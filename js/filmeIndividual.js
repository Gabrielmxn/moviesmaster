function getDoFilmes() {
	...
	//buscar  API
	xmlHttp[0].open('GET', 'https://api.themoviedb.org/3/movie/' + idDoFilme + '?api_key=bd9f051458a4c87fe4c873ef463542e3&language=pt-BR');
		...
		xmlHttp[o].onreadystatechange = () => {
			...
			if(xmlHttp[0].readyState == 4 & xmlHttp[0].status == 200 & xmlHttp[1].readyState == 4 & xmlHttp[1].status == 200){
				...
				//pecorrendo o objeto e recuperando os seus valores.
					...
					divCol2.className = 'col-4 col-sm-2 col-md-2 col-lg-3 p-0 m-0 align-self-center img' 
					divCol2.id = "coluna";

					//Criando uma coluna para outras informações
					let divCol = document.createElement('div');
					divCol.className = 'col-8 col-sm-10 col-md-10 col-lg-9 align-self-center'
					...
					if(tamanhoDaTela > 768)
						...
						if(item.poster_path){
							...
							elemento.style.background = "url(https://image.tmdb.org/t/p/original/" + item.backdrop_path + ") center center";
					...
					botao.id = "botao";
					botao.className = "btn my-3 ml-0 float-left"
					botao.innerHTML = "<i class='fas fa-list text-white'></i>";
					botao.onclick = function() {enviar()}

					/*let botao2 = document.createElement('button');
					botao2.id = "botao";
					botao2.className = "btn my-3 ml-3 float-left"
					botao2.innerHTML = "<i class='fas fa-list text-white'></i>";
*/
